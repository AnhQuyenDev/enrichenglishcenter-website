export const teachers_data = [
    {
        "id": 1,
        "avatar": "/images/cohaoavatar.jpg",
        "name": "Nguyễn Thị Bích Hảo",
        "subject": "CEO & Founder",
        "bio": "Nhà sáng lập và Giám đốc điều hành của Enrich, người dẫn dắt và truyền cảm hứng cho toàn bộ đội ngũ.",
        "introduction": "Với tầm nhìn chiến lược và đam mê cháy bỏng với giáo dục, cô Nguyễn Bích Hảo đã sáng lập Enrich với mong muốn tạo ra một môi trường học tập Anh ngữ chất lượng cao và sáng tạo. Cô là người định hướng phát triển, đảm bảo mọi hoạt động của trung tâm đều hướng tới lợi ích và sự tiến bộ của học viên.",
        "experience": [
            "Tốt nghiệp loại Giỏi - Đại học Ngoại ngữ Huế, chuyên ngành Sư Phạm Tiếng Anh.",
            "Sáng lập và điều hành Trung tâm Anh ngữ Enrich.",
            "Nhiều năm kinh nghiệm trong lĩnh vực quản lý giáo dục và phát triển trung tâm."
        ],
        "certifications": [
            "Chứng chỉ IELTS C1",
        ],
        "achievements": [
            {
                "year": 2024,
                "title": "Sáng lập và xây dựng thành công thương hiệu Enrich."
            }
        ],
        "contact": {
            "email": "bichhao.ceo@gmail.com",
            "phone": "0933 701 555",
            "facebook": "https://www.facebook.com/bichhao.nguyen.520"
        }
    },
    {
        "id": 2,
        "type": "VN", // <-- Thêm trường "type" để phân biệt giáo viên Việt Nam
        "avatar": "/images/van-thi-sao.jpg",
        "name": "Văn Thị Sao",
        "subject": "Ngôn ngữ Anh",
        "bio": "Giảng viên Tiếng Anh trẻ tuổi, nhiệt huyết với bằng chứng nhận năng lực ngoại ngữ C1.",
        "introduction": "Văn Thị Sao là một giáo viên tận tâm với chuyên ngành Ngôn ngữ Anh. Với kinh nghiệm giảng dạy tại Trung tâm Anh ngữ, cô luôn nỗ lực tạo ra một môi trường học tập tích cực và hiệu quả. Cô đặc biệt chú trọng vào việc phát triển kỹ năng giao tiếp và ngữ pháp cho học viên.",
        "experience": [
            "Giảng dạy tại Trung tâm Anh ngữ."
        ],
        "certifications": [
            "Chứng nhận Năng lực Ngoại ngữ C1"
        ],
        "achievements": [],
        "contact": {
            "email": "vanthisao2003@gmail.com",
            "phone": "0815 250 531",
            "facebook": "https://www.facebook.com/share/1EhSVoM277/?mibextid=wwXlfr"
        }
    },
    {
        "id": 3,
        "type": "VN", // <-- Thêm trường "type" để phân biệt giáo viên Việt Nam
        "avatar": "/images/phan-thi-thuy-trang.jpg",
        "name": "Phan Thị Thuỳ Trang",
        "subject": "Sư Phạm Tiếng Anh",
        "bio": "Giáo viên trẻ, nhiệt huyết với thành tích học tập ấn tượng và chứng chỉ IELTS C1.",
        "introduction": "Phan Thị Thuỳ Trang là một giáo viên chuyên ngành Sư phạm Tiếng Anh đến từ Đại học Ngoại ngữ Huế. Với kinh nghiệm giảng dạy tại nhiều trung tâm và thành tích học tập xuất sắc, cô Trang mang đến cho học viên một phương pháp học tập hiệu quả, giúp học viên phát triển toàn diện các kỹ năng tiếng Anh.",
        "experience": [
            "Giảng dạy tại Trung tâm Anh ngữ Âu Mỹ.",
            "Giảng dạy tại Trung tâm Anh ngữ Phương Mỹ."
        ],
        "certifications": [
            "Chứng chỉ IELTS C1"
        ],
        "achievements": [
            {
                "year": 1,
                "title": "Giải nhì môn tiếng Anh thành phố Huế"
            },
            {
                "year": 2,
                "title": "Điểm thi xét tuyển đại học môn tiếng Anh 9.75"
            }
        ],
        "contact": {
            "email": "trangphan040902@gmail.com",
            "phone": "0372 717 029",
            "facebook": "https://www.facebook.com/share/1C1aWDxfie/?mibextid=wwXIfr"
        }
    },
    {
        "id": 4,
        "type": "VN", // <-- Thêm trường "type" để phân biệt giáo viên Việt Nam
        "avatar": "/images/hoang-thi-my-linh.jpg",
        "name": "Hoàng Thị Mỹ Linh",
        "subject": "Ngôn ngữ Anh",
        "bio": "Giảng viên tiếng Anh có kinh nghiệm trợ giảng và giảng dạy 3 năm cho mọi lứa tuổi.",
        "introduction": "Hoàng Thị Mỹ Linh là một giáo viên tiếng Anh giàu kinh nghiệm với 3 năm giảng dạy cho nhiều đối tượng học viên khác nhau. Cô sở hữu các chứng chỉ giảng dạy quốc tế như Tesol và C1, đảm bảo chất lượng giảng dạy và hiệu quả học tập cho học viên.",
        "experience": [
            "3 năm trợ giảng và giảng dạy tiếng Anh cho mọi lứa tuổi."
        ],
        "certifications": [
            "Chứng chỉ Tesol",
            "Chứng chỉ C1"
        ],
        "achievements": [],
        "contact": {
            "email": "mylinhhoang2002@gmail.com",
            "phone": "0339635116"
        }
    },
    // --- VÍ DỤ GIÁO VIÊN NƯỚC NGOÀI ---
    {
        "id": 5,
        "type": "NF", // <-- Thêm trường "type" để phân biệt giáo viên nước ngoài
        "avatar": "https://thumb.photo-ac.com/59/598f5b956f414ef4c3096ab6ff5e7e7a_t.jpeg",
        "name": "John Smith",
        "subject": "Native English Teacher",
        "bio": "Giáo viên bản ngữ đến từ Anh với chứng chỉ CELTA và hơn 5 năm kinh nghiệm giảng dạy tại châu Á.",
        "introduction": "Thầy John Smith mang đến một phương pháp tiếp cận năng động, tập trung vào kỹ năng giao tiếp và phát âm chuẩn. Thầy tin rằng việc học tiếng Anh phải là một trải nghiệm thú vị và có tính tương tác cao. Các lớp học của thầy luôn tràn ngập năng lượng tích cực.",
        "experience": [
            "5+ năm kinh nghiệm giảng dạy tiếng Anh tại Việt Nam và Thái Lan.",
            "Chuyên dạy các lớp giao tiếp và luyện phát âm cho người lớn."
        ],
        "certifications": [
            "CELTA (Certificate in English Language Teaching to Adults)",
            "Bachelor's Degree in Linguistics, University of Manchester"
        ],
        "achievements": [],
        "contact": {
            "email": "john.smith@enrich.edu.vn",
            "phone": "N/A"
        }
    },
    {
        "id": 6,
        "type": "NF", // <-- Thêm trường "type" để phân biệt giáo viên nước ngoài
        "avatar": "https://images.unsplash.com/photo-1607746882042-944635dfe10e?w=400&h=400&fit=crop&crop=faces&utm_source=chatgpt.com",
        "name": "Emily Jones",
        "subject": "Young Learners Specialist",
        "bio": "Chuyên gia đào tạo tiếng Anh cho trẻ em đến từ Mỹ, có chứng chỉ TEFL và đam mê giáo dục sớm.",
        "introduction": "Cô Emily Jones có khả năng đặc biệt trong việc kết nối và truyền cảm hứng cho các học viên nhỏ tuổi. Cô sử dụng các phương pháp học qua trò chơi, bài hát và các hoạt động sáng tạo để giúp trẻ tiếp thu ngôn ngữ một cách tự nhiên và vui vẻ nhất.",
        "experience": [
            "Giảng dạy tiếng Anh cho trẻ em tại các trường quốc tế.",
            "Thiết kế chương trình học tiếng Anh cho lứa tuổi mầm non."
        ],
        "certifications": [
            "TEFL (Teaching English as a Foreign Language)",
            "Bachelor's in Early Childhood Education, University of California"
        ],
        "achievements": [],
        "contact": {
            "email": "emily.jones@enrich.edu.vn",
            "phone": "N/A"
        }
    },
    {
        "id": 7,
        "type": "NF", // <-- Thêm trường "type" để phân biệt giáo viên nước ngoài
        "avatar": "https://image.tinnhanhchungkhoan.vn/w2400/Uploaded/2025/wpxlcdjwi/2021_02_12/adam-calver-3266.jpg",
        "name": "David Chen",
        "subject": "Business English Instructor",
        "bio": "Giảng viên tiếng Anh thương mại từ Canada với kinh nghiệm làm việc trong môi trường doanh nghiệp quốc tế.",
        "introduction": "Thầy David Chen chuyên về các khóa học tiếng Anh dành cho người đi làm, tập trung vào kỹ năng thuyết trình, đàm phán, viết email và giao tiếp trong công việc. Thầy giúp học viên tự tin sử dụng tiếng Anh như một công cụ đắc lực cho sự nghiệp.",
        "experience": [
            "Đào tạo tiếng Anh cho nhân sự tại các tập đoàn đa quốc gia.",
            "4+ năm kinh nghiệm giảng dạy Business English."
        ],
        "certifications": [
            "TESOL (Teaching English to Speakers of Other Languages)",
            "Master of Business Administration (MBA)"
        ],
        "achievements": [],
        "contact": {
            "email": "david.chen@enrich.edu.vn",
            "phone": "N/A"
        }
    }
]